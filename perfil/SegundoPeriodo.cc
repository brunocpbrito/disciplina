    ...
    if (aluno->getNome() == "turma") {
        ...
        if (pegarEspera) {
            if (turma.getLength() < capacidadeFila && !filaEspera.isEmpty()) {
                EV << "\n Turma com " << turma.getLength()  << " alunos, restando "   << (capacidadeFila - turma.getLength())  << " vagas. Pegando alunos da fila de espera ("   << filaEspera.getLength()   << ") do Segundo Periodo, ate completar as vagas. " << endl;
...
void SegundoPeriodo::colocarFila(Aluno *aluno) {
    ...
    if (turma.getLength() < capacidadeFila) {
        ...
        turma.insert(aluno);
        if (turma.getLength() == capacidadeFila) {
            EV << "\n Turma do Segundo Periodo com "<< turma.getLength() <<" completa, o resto vai para a fila de espera. "  << endl;
        }
    } else {
